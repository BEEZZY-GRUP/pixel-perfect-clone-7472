  ...
  <section id="methodology" className="px-7 py-20 md:px-14 md:py-[140px]">
    ...
    <div className="mt-20 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 border border-border">
      {steps.map((step, i) => (
        ...
        >
          <div className="absolute inset-0 bg-foreground origin-bottom scale-y-0 group-hover:scale-y-100 transition-transform duration-500 ease-[cubic-bezier(.25,.46,.45,.94)] z-0" />
          <div className="relative z-[1]">
            <div className="font-display text-[4rem] font-light text-foreground/[.15] leading-none mb-10 group-hover:text-background/[.15] transition-colors">
              {step.num}
            </div>
            <div className="font-bold text-[.8rem] tracking-[.12em] uppercase mb-4 group-hover:text-background transition-colors">
              {step.title}
            </div>
            <div className="text-[.8rem] leading-[1.8] text-muted-foreground group-hover:text-background transition-colors">
              {step.desc}
            </div>
          </div>
        </div>
